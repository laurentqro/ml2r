class RiskFactorDefinition < ApplicationRecord
  ...
  validates :description, uniqueness: { scope: [ :risk_factor_type, :category ], message: "must be unique within the same type (person or company) and category (client_risk, products_and_services_risk, distribution_channel_risk, transaction_risk)" }
